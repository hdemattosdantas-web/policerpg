...
export async function ensureUserDoc({ uid, nome_policial }) {
  ...
  const data = {
    ...
    total_patrulhas: 0,
    ocorrencias_sucesso: 0,
    dinheiro_acumulado: 500, // Começa com o bônus de adesão
    itens_comprados: 0,
    isPremium: false,
  }
...
export async function applyPatrolReward(uid, { xpDelta, saldoDelta }) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    const totalPatrulhasAtual = Number.isFinite(data.total_patrulhas) ? data.total_patrulhas : 0
    const dinheiroAcumuladoAtual = Number.isFinite(data.dinheiro_acumulado) ? data.dinheiro_acumulado : 0

    const novoXp = xpAtual + xpDelta
    const novoSaldo = saldoAtual + saldoDelta
    const novoTotalPatrulhas = totalPatrulhasAtual + 1
    const novoDinheiroAcumulado = dinheiroAcumuladoAtual + saldoDelta
    const patente_atual = verificarPromocao(novoXp).nome

    tx.update(ref, {
      xp: novoXp,
      saldo: novoSaldo,
      total_patrulhas: novoTotalPatrulhas,
      dinheiro_acumulado: novoDinheiroAcumulado,
      patente_atual,
    })
  })
}

export async function updateEstatistica(uid, campo, valor = 1) {
  const ref = userRef(uid)
  ...
  await runTransaction(db, async (tx) => {
    ...
    const data = snap.data()
    const atual = Number.isFinite(data[campo]) ? data[campo] : 0
...
export async function incrementarOcorrenciasSucesso(uid) {
  await updateEstatistica(uid, 'ocorrencias_sucesso', 1)
}

export async function incrementarItensComprados(uid) {
  await updateEstatistica(uid, 'itens_comprados', 1)
}
